...
# Install missing dependencies at runtime (non-blocking)
...
def install_deps():
    ...
    try:
        subprocess.run([sys.executable, '-m', 'pip', 'install', '--only-binary=:all:', 'numpy>=2.1.0,<3.0.0'], check=False, timeout=60)
        subprocess.run([sys.executable, '-m', 'pip', 'install', '--only-binary=:all:', 'spacy>=3.7.0,<4.0.0'], check=False, timeout=60)
        subprocess.run([sys.executable, '-m', 'pip', 'install', '--only-binary=:all:', 'textacy>=0.12.0,<1.0.0'], check=False, timeout=60)
        print('✅ Dependencies installed')
    except Exception as e:
        print(f'⚠️ Some deps failed: {e}')

install_deps()
" || echo "⚠️ Dependency installation failed, continuing..."
